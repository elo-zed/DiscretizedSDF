...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, load_iteration):
    ...
    with torch.no_grad(): 
        ...
        if opt.lambda_distortion > 0. and iteration > opt.dist_from_iteration:
        ...
        if opt.lambda_proj > 0. and iteration > opt.proj_from_iteration:
            ...
            gaussian_proj_depth = torch.nn.functional.grid_sample(input=render_pkg['depth'].unsqueeze(0),
        ...
        with torch.no_grad():
            ...
            training_report(tb_writer, iteration, Ll1, loss, losses_extra, l1_loss, iter_end - iter_start, 
                            testing_iterations, scene, render_fn, (pipe, test_bg), {"is_training": False})
...
def training_report(tb_writer, iteration, Ll1, loss, losses_extra, l1_loss, elapsed, 
                    testing_iterations, scene, renderFunc, renderArgs, renderArgDict={}):
    ...
    if iteration in testing_iterations:
        ...
        render_set(scene.model_path, 'test', iteration, scene.getTestCameras(), scene, renderArgs[0], background=torch.tensor([1,1,1], dtype=torch.float32, device="cuda"))
        if tb_writer:
